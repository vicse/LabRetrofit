package com.ore.vicse.labretrofit.activities;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void callRegister(View view){
        ...
        String motivos = motivoInput.getText().toString();
        String tipo = tipoInput.getText().toString();

        if (correo.isEmpty() || motivos.isEmpty() || tipo.isEmpty()) {
            Toast.makeText(this, "Todos los son campos requeridos", Toast.LENGTH_SHORT).show();
            return;
        }

        ApiService service = ApiServiceGenerator.createService(ApiService.class);

        Call<Solicitud> call = null;

        if (mediaFileUri == null) {
            // Si no se incluye imagen hacemos un envío POST simple
            call = service.createSolicitud(correo, tipo, motivos);
        } else {
            ...
            RequestBody motivosPart = RequestBody.create(MultipartBody.FORM, motivos);
            RequestBody tipoPart = RequestBody.create(MultipartBody.FORM, tipo);

            call = service.createSolicitudWithImage(correoPart, motivosPart, tipoPart, imagenPart);
        }
